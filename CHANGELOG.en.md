...
- Port availability is decided up front and told apart as reusable / owned by another program / a dsh this machine cannot enter (a Windows-side instance, or one under another `DSH_HOME`) / below `1024`, instead of being discovered by a failed start.
- A port typed by hand is remembered and offered as the next default.
- A check for updates to this application itself. Startup asks this repository's GitHub Releases in parallel with everything else and offers *Download and install* above the version columns: the installer for this platform is picked by name (`setup.exe` on Windows, `dmg` on macOS, `deb` on Linux), downloaded, checked against the release's published `SHA256SUMS`, and only then handed to the system installer. A release with nothing installable here degrades to *Open the release page*; a mismatched checksum, or a release that publishes none, is refused and nothing is written.
- A marketplace entry, `plugins/dsh-desktop-app/`. A dependency-free installer stub declaring the `dsh.bundle` manifest the listing requires: on the **first** dsh start after installation it picks this platform's installer from the latest release, downloads `SHA256SUMS`, writes the installer only once the digest matches, and hands it to the system installer. A machine that already has the shell, one with no desktop session (`CI`, or Linux without `DISPLAY`/`WAYLAND_DISPLAY`), and a platform with no installer at all (`linux/arm64`, `win32/arm64`) are each only told where to look. It installs nothing silently and imports no harness API.
...
- The dialog's "installed at" line now shows the resolved dsh launcher; it used to show the loopback URL.
...
- The release workflow gained a `stub` job: it runs `npm pack`, attaches the result to the release under the **version-free** name `dsh-xswt-tauriapp-plugin.tgz`, and asserts the tarball really carries `dsh.bundle` and the `cordis.patch.yml` it names. The name must stay version-free, because `releases/latest/download/<name>` resolves `latest` at request time but takes the filename literally. The tag contract now covers five version fields instead of four (adding `plugins/dsh-desktop-app/package.json`).
- Both changelogs carried two `## Unreleased` sections, and `scripts/release-notes.mjs` takes only the first match — so 0.0.8's notes would have dropped the whole "updates to this application itself" section. The two sections are now one.
