...
- A marketplace entry, `plugins/dsh-desktop-app/`: a dependency-free installer stub declaring the `dsh.bundle` manifest the listing requires. On the **first** dsh start after it is installed, it picks this platform's installer from the latest release, downloads `SHA256SUMS`, writes the installer only once the digest matches, and hands it to the system installer. A machine that already has the shell, one with no desktop session (`CI`, or Linux without `DISPLAY`/`WAYLAND_DISPLAY`), and a platform with no installer at all (`linux/arm64`, `win32/arm64`) are each only told where to look. It installs nothing silently and imports no harness API — no tool, no UI, no dsh internal.
...
- The release workflow gained a `stub` job: it runs `npm pack`, attaches the result to the release under the **version-free** name `dsh-xswt-tauriapp-plugin.tgz`, and asserts the tarball really carries `dsh.bundle` and the `cordis.patch.yml` it names. The version-free name is load-bearing: `releases/latest/download/<name>` resolves `latest` at request time but takes the filename literally, so a versioned name would 404 silently once the next tag was cut. The tag contract now covers five version fields instead of four (adding `plugins/dsh-desktop-app/package.json`).
...
- Both `CHANGELOG.md` and `CHANGELOG.en.md` carried two `## Unreleased` sections, and `scripts/release-notes.mjs` takes only the first match — so 0.0.8's notes would have dropped the whole "updates to this application itself" section. The two sections are now one.
