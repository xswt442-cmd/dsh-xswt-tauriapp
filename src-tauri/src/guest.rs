...
use dsh_xswt_tauriapp_core::server::Session;
...
/// Zoom bounds for the guest window.
...
fn zoom_by(app: &AppHandle, shell: &SharedShell, factor: f64) {
    let zoom = match shell.lock() {
        Ok(mut guard) => {
            guard.zoom = (guard.zoom * factor).clamp(MIN_ZOOM, MAX_ZOOM);
            guard.zoom
        }
        Err(_) => return,
    };
