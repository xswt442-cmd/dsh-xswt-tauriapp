...
- 启动时可以选择端口。弹窗在版本三栏下方给出端口框，灰色数字是默认值（`3080`–`3129` 中第一个空闲端口，手输过的端口优先）；确认即按默认启动，也可以输入任意端口。
- 端口可用性当场判定并区分四种情况：可复用 / 被别的程序占用 / 是 dsh 但本机无法接手其会话（例如在 Windows 侧启动，或用了另一个 `DSH_HOME`）/ 低于 `1024`，不再等到启动失败。
- 记住手输过的端口，作为下次的默认值。
- 外壳自身的更新检查：启动时并行查询本仓库的 GitHub Releases，有新版本就在弹窗顶部提示并给出「下载并安装」——按平台挑安装包（Windows `setup.exe`、macOS `dmg`、Linux `deb`），比对发布页的 `SHA256SUMS` 通过后才交给系统安装器；没有本平台安装包时退化为「打开发布页」，校验不通过或该发布没有校验文件则拒绝下载并**不落盘**。与 dsh 的更新互不影响，各自记录「不再提示」。
- 插件市场入口 `plugins/dsh-desktop-app/`：一个零依赖的引导插件，声明市场要求的 `dsh.bundle`，装上之后**首次**启动 dsh 时按平台从最新 release 挑出安装包，先取 `SHA256SUMS`、校验通过才写盘，再交给系统安装器打开；已经装好外壳的机器、没有桌面会话的机器（`CI`，或 Linux 上没有 `DISPLAY`/`WAYLAND_DISPLAY`）、本平台没有安装包（`linux/arm64`、`win32/arm64`）三种情况都只打印一句说明。它不静默安装，也不导入任何 harness API —— 不注册工具、不注入界面、不读 dsh 内部结构。

### 变更

- 弹窗改为每次启动都出现（标题「启动 dsh」，主按钮「打开 dsh」）。此前它只在有更新时出现，因此没有可以选端口的时机。更新检查与端口扫描并行，确认不等待 registry。
- 发现候选端口改为从 `$DSH_HOME/launcher/logs` 的日志文件名派生，不再扫描整个 `3080`–`3129`；扫描另用更短的连接超时。这既让非标准端口下次仍能被发现，也消除了 Windows 上无服务时约 40 秒的冷启动。
- 发布工作流新增 `stub` 任务：`npm pack` 出市场引导包，以**不带版本号**的名字 `dsh-xswt-tauriapp-plugin.tgz` 附到 release 上，并断言包里确实带着 `dsh.bundle` 与它指向的 `cordis.patch.yml`。名字不带版本号是必需的：`releases/latest/download/<name>` 只解析 `latest`，文件名照字面取，带版本号的名字会在下一个 tag 发布时静默 404。版本一致性检查从四个字段扩到五个（新增 `plugins/dsh-desktop-app/package.json`）。

### 修复

- 菜单或托盘构建失败不再阻止应用启动。两者是便利设施，此前 `setup` 会把它们的失败当成启动失败 —— 在没有状态栏宿主的桌面上会打不开，而 macOS 菜单那条路径从未运行过。
- 对话框的「已安装位置」此前填的是回环 URL，现改为实际的 dsh 启动器路径。
- 版本比较不再可能把当前版本当成新版本提示：GitHub 的 tag 带 `v` 前缀（`v0.0.7`），而 dsh 那套比较函数在解析失败时会退化成字符串比较，`"v0.0.7" != "0.0.7"` 就会被判定为"有更新"。自更新这条改为先剥离前缀、且要求两边都能解析成 semver。
- `CHANGELOG.md` 与 `CHANGELOG.en.md` 里各有两个 `## Unreleased` 段，而 `scripts/release-notes.mjs` 只取第一个匹配的段，0.0.8 的发布说明会因此整段丢掉「外壳自身的更新」。两个 `## Unreleased` 已合并为一段。
