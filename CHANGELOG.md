...
- 更新失败的提示不再是一行乱码：子进程输出先按 UTF-8 解，解不开再按机器的 OEM 代码页解；此前 Windows 自己的中文报错会变成替换字符。
- 外壳自身的更新在 Linux / WSL 上不再谎报成功：交给系统打开时会等 `xdg-open` 的结果，没有 `.deb` 处理器时（退出码 3）明确说"系统里没有能打开它的程序"并给出 `sudo apt install <路径>`；Windows 仍不等待，因为 `explorer` 的退出码与文件无关。
