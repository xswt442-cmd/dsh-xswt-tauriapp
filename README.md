...
- **外壳自身的更新**：启动时另查本仓库的 GitHub Releases，有新版本时在弹窗顶部提示并给出「下载并安装」——按平台挑对应安装包，比对发布页的 `SHA256SUMS` 校验后才交给系统安装器打开。与上面那条 dsh 的更新是两回事，各自有独立的「不再提示」记录。
...
与 dsh 的更新是两条独立的路：dsh 来自 npm，外壳来自本仓库的 GitHub Releases。启动时两者并行检查，互不阻塞。
...
版本比较要求两边都能解析成 semver，且 tag 的 `v` 前缀会先剥掉。这里**刻意不复用** dsh 那套比较函数的字符串回退：对版本流来说"字符串不同即更新"是合理的猜测，对更新器来说它会让你把**当前正在运行的版本**当成新版提示一遍。
